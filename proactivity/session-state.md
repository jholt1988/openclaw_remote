...
## Next Useful Move
Either continue with the next `LeaseService` coverage slice (renewal offers/notices/tenant notices) or start cleaning repo-wide type/lint drift so full gates become meaningful again.

## Recent Proactive Action
2026-04-30: Added `tenant_portal_backend/src/lease/lease.service.spec.ts` covering create lease validation/org guard/history, status update org guard/history, and renewal-window query behavior. Targeted Jest passed: 6 suites, 35 passed.
